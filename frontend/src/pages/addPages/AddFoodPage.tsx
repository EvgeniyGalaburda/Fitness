...
export const AddFoodPage = () => {
    ...
    const {mutate: addFood} = useMutation({
        mutationFn: async ({title, energyValue, weight}: {title: string, energyValue: number, weight: number}) => {
            const res = await axios.post("http://localhost:5000/api/food/add", {title, energyValue, weight}, {withCredentials: true});
            return res;
        },
        onSuccess: async () => {
    ...
    const handleSelect = (e: {target: {value: string}}) => {
        setFormData({...formData, title: JSON.parse(e.target.value).title, energyValue: JSON.parse(e.target.value).energyValue});
    ...
    <div className={style.page}>
        ...
        <form onSubmit={e => {e.preventDefault();}}>
            ...
            <select onChange={handleSelect} name="title" >
                {foodData.map((f,i) => (
                    <option key={i} value={JSON.stringify({title: f.title, energyValue: f.energyValue})}>{f.title}</option>
                ))}
            </select>
            <h2>Вага продукту (г)</h2>
            <input onChange={setWeight} type="number" value={formData.weight} />
            <Button onClick={() => addFood(formData)}>Додати<FaPlus /></Button>
        </form>
    </div>
  )
}
